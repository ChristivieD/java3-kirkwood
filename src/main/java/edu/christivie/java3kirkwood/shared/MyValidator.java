...
public class MyValidator {
    ...
    // required a characters, 3 of 4 (uppercase, lowercase, number, or character)
    ...
    public static Pattern languagePattern = Pattern.compile("^(en-US|fr-FR|es-ES|ar)$");
    public static Pattern phonePattern = Pattern.compile("^\\D?(\\d{3})\\D?\\D?(\\d{3})\\D?(\\d{4})$");
    // Status selection
    public static final Pattern statusPattern = Pattern.compile("^(inactive|active|locked)$");

    // Privileges selection
    public static final Pattern privilegesPattern = Pattern.compile("^(user|premium|admin)$");
}
